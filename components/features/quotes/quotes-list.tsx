...
export function QuotesList({ userId, quotes: initialQuotes }: { userId: string; quotes: BookQuote[] }) {
    ...
    const toggleFavorite = async (quoteId: string, currentFavorite: boolean) => {
        const { error } = await supabase.from("book_quotes").update({ is_favorite: !currentFavorite }).eq("id", quoteId)
        ...
        <div className="space-y-6">
            <div className="flex gap-2 border-b">
                <button
                    ...
                    className={`px-4 py-2 font-medium transition-colors border-b-2 ${filter === "all" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
                >
                    All Quotes ({quotes.length})
                </button>
                <button
                    onClick={() => setFilter("favorites")}
                    className={`px-4 py-2 font-medium transition-colors border-b-2 flex items-center gap-2 ${filter === "favorites" ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
                >
                    ...
                    <Card key={quote.id} className="p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between gap-4 mb-4">
                            <div className="flex-1">
                                <Link href={`/dashboard/reader/${quote.books?.id ?? "#"}`} className="font-semibold hover:underline">
                                    {quote.books?.title ?? "Unknown Book"}
                                </Link>
                                <p className="text-sm text-muted-foreground">by {quote.books?.author ?? "Unknown Author"}</p>
                            </div>
                        ...
                        {(quote.page_number != null || quote.chapter) && (
                            <div className="flex gap-4 text-sm text-muted-foreground mb-2">
                                {quote.page_number != null && <span>Page {quote.page_number}</span>}
