...
import { BookOpen, ArrowRight, ArrowLeft, Sparkles, Heart, Star, TrendingUp, ImagePlus } from "lucide-react"
          ...
          <CardContent className="px-12 pb-12">
            <AnimatePresence mode="wait">
              {currentStep === 1 && (
                ...
                >
                  ...
                  <div className="space-y-4">
                    <h3 className="text-3xl font-black italic tracking-tighter">Your Journey Begins.</h3>
                    <p className="text-muted-foreground text-lg leading-relaxed max-w-sm mx-auto">
                      Trove is more than a reader. It's your sanctuary for wisdom and discovery. Let's make it yours.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-3 justify-center pt-6">
                    {["Curated Feed", "Naira Payments", "Social Clubs"].map(item => (
                      <div key={item} className="bg-primary/5 border border-primary/10 px-4 py-2 rounded-full text-xs font-bold text-primary shadow-sm">
                        {item}
                      </div>
                    ))}
                  </div>
              ...
              {currentStep === 2 && (
                ...
                >
                  <div className="space-y-4">
                    <Label htmlFor="nickname" className="text-sm font-black uppercase tracking-widest text-muted-foreground">
                      Public Handle
                    </Label>
                    <Input
                      id="nickname"
              ...
              {currentStep === 3 && (
                ...
                >
                  <div className="grid grid-cols-2 gap-8 items-start">
                    <div className="space-y-4">
                      <Label className="text-sm font-black uppercase tracking-widest text-muted-foreground">Personal Photo</Label>
                      <div className="relative group overflow-hidden bg-muted/30 rounded-[2rem] aspect-square flex flex-col items-center justify-center border-2 border-dashed border-border group-hover:border-primary/50 transition-all">
                        ...
                        />
                        {uploadingAvatar && <div className="absolute inset-0 bg-background/60 backdrop-blur-sm flex items-center justify-center text-xs font-bold">Optimizing...</div>}
                      </div>
                    </div>

                    <div className="space-y-4">
                      <Label className="text-sm font-black uppercase tracking-widest text-muted-foreground">Preset Identity</Label>
                      <div className="grid grid-cols-3 gap-3">
                        {PRESET_AVATARS.map(avatar => (
                          <button
                            key={avatar.id}
                            onClick={() => { setAvatarChoice(avatar.id); setAvatarUrl(null); }}
                            className={`aspect-square rounded-2xl border-4 transition-all overflow-hidden relative group ${avatarChoice === avatar.id ? 'border-primary shadow-xl scale-105' : 'border-transparent opacity-60 hover:opacity-100'
                              }`}
                          >
                            <Image
                      ...
                      <p className="text-[10px] text-muted-foreground font-medium italic">Hand-crafted presets to kickstart your journey.</p>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {currentStep === 4 && (
                <motion.div
                  ...
                  className="space-y-6"
                >
                  <Label className="text-sm font-black uppercase tracking-widest text-muted-foreground px-2">Knowledge Domains</Label>
              ...
              {currentStep === 5 && (
                <motion.div
                  ...
                  className="space-y-6"
                >
                  <Label className="text-sm font-black uppercase tracking-widest text-muted-foreground px-2">Existing Legends</Label>
                  <div className="flex gap-3">
                    ...
                    />
                    <Button onClick={addFavoriteBook} type="button" size="lg" className="rounded-2xl h-14 px-8 font-bold">
                      Record
                    </Button>
                  </div>
              ...
              {currentStep === 6 && (
                ...
                >
                  ...
                  <div className="space-y-4">
                    <h3 className="text-4xl font-black italic tracking-tighter">Welcome Home traveler.</h3>
                    <p className="text-muted-foreground text-lg leading-relaxed max-w-sm mx-auto">
                      Your Trove has been initialized. A world of knowledge is now at your fingertips.
                    </p>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex justify-between mt-12 pt-8 border-t border-border/30">
              ...
              {currentStep < STEPS.length ? (
                <Button onClick={handleNext} disabled={loading || (currentStep === 2 && !nickname.trim())} className="rounded-full shadow-2xl shadow-primary/30 h-14 px-10 text-lg font-black italic">
                  Continue
                  <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              ) : (
                <Button onClick={handleComplete} disabled={loading} className="rounded-full shadow-2xl shadow-primary/40 h-16 px-12 text-xl font-black italic bg-primary scale-105 hover:scale-110 transition-transform">
